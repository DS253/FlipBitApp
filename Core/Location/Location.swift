...
class Location: NSObject, GenericBaseModel, NSCoding {
    ...
    func isInRange(to location: Location, by distance: Double) -> Bool {
        return (self.distance(to: location) / 1609.34 <= distance)
    }
    
    func stringDistance(to location: Location) -> String {
        let distance = Int(self.distance(to: location) / 1609.34)
        return String(distance) + " miles"
    }
    
    fileprivate func distance(to otherLocation: Location) -> Double {
