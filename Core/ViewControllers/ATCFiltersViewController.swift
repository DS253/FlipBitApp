...
class ATCFiltersViewController: ATCGenericCollectionViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    init(uiConfig: ATCUIGenericConfigurationProtocol,
        ...
        if let localFiltersStore = localFiltersStore,
            (localFiltersStore.allFilters.count + localFiltersStore.filtersByCategory.count) > 0 {
            if let categoryID = category?.id {
                if let filters = localFiltersStore.filtersByCategory[categoryID] {
                    filtersDataSource  = ATCGenericLocalDataSource<ATCSelectFilter>(items: filters)
                }
            } else if localFiltersStore.allFilters.count > 0 {
                filtersDataSource  = ATCGenericLocalDataSource<ATCSelectFilter>(items: localFiltersStore.allFilters)
            }
        }
    ...
    @objc private func didTapDoneButton() {
        if let ds = self.genericDataSource {
            ...
            if let category = category {
                localFiltersStore?.filtersByCategory[category.id] = newFilters
            } else {
                localFiltersStore?.allFilters = newFilters
            }
